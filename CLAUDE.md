# DevTools — Plugin Context for Claude

> Plugin for the **AlfacodeTeam PhpServicePlatform** (Sentinel) kernel.
> Package `alfacode-team/hkm-plugin-dev-tools` · namespace `Plugins\DevTools\` · solves `dev.tooling`
>
...
| `exposes` | — |
| `emits` | — |
| Activation | `require-dev` — **never enable in production** |
...
`config[]` is also what `hkm plugins enable dev-tools` seeds into the project's
...
Scaffolding and introspection CLI. **`require-dev` — never enable in
production.**

| Command | Does |
|---|---|
| `make:plugin`, `make:service` | GDA scaffolding |
| `module:list`, `module:info <name>` | read `module.json` as the source of truth |
| `routes:list` | compiled routes, with collision detection |
| `project:list` | registered projects |
| `config:show`, `config:clear` | compiled config manifest |

Introspection commands read the **compiled manifests and `module.json`**, never a
hand-maintained list. That is what keeps them honest when a plugin changes.

```
✗ Enabling this in a production bootstrap
✗ A generator that emits code violating the five access rules — scaffolding is
  where a bad pattern gets copied a hundred times
✗ config:show printing secret values in full — redact
✗ Duplicating a check the boot pipeline already performs — a second definition of
  one rule drifts from the first
```
